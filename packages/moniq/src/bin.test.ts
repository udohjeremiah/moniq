...
describe("bin", () => {
  ...
  it("degrades gracefully on malformed shell input", () => {
    const match = bin("eslint");
    expect(match('eslint "unclosed double')).toBe(true);
    expect(match("eslint 'unclosed single")).toBe(true);
    expect(match("eslint | pipe")).toBe(true);
    expect(match("")).toBe(false);
  });
});
