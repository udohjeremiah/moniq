...
/**
 * Represents a package discovered in the workspace.
 */
...
/**
 * Discovers all packages in a monorepo workspace.
 *
 * Detects the package manager by checking `npm_config_user_agent`,
 * the project's lock file (`package-lock.json`, `pnpm-lock.yaml`, or
 * `yarn.lock`), or the `packageManager` field in the root `package.json`.
 * Runs the appropriate discovery command and normalises the output into
 * a uniform list of {@link Package} objects.
 *
 * Supported package managers: pnpm, npm, yarn.
 *
 * @param root - Absolute path to the workspace root directory.
 * @returns Array of discovered packages, each with an absolute `path`.
 */
