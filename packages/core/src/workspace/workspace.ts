...
export async function discoverWorkspace(root: string) {
  ...
  switch (pm) {
    case "bun": {
      packages = await resolveBunWorkspaces(workspaceRoot);
      break;
    }
    case "deno": {
      packages = await resolveDenoWorkspaces(workspaceRoot);
      break;
    }
    case "npm": {
      const output = runPmSync(
        "npm",
        ["ls", "--workspaces", "--all", "--json", "--depth", "0"],
        workspaceRoot,
      );
      const tree = JSON.parse(output) as {
        dependencies?: Record<string, { path?: string; resolved?: string }>;
      };
      packages = Object.values(tree.dependencies ?? {})
        .map((entry) => ({ path: npmPackagePath(workspaceRoot, entry) }))
        .filter((entry): entry is Package => typeof entry.path === "string");
      break;
    }
    case "pnpm": {
      const output = runPmSync(
      ...
      const entries = JSON.parse(output) as { path: string }[];
      packages = entries.map((entry) => ({ path: entry.path }));
      break;
    }
    case "yarn": {
      const output = runPmSync(
      ...
      packages = entries.map((entry) => ({
        path: path.resolve(workspaceRoot, entry.location),
      }));
      break;
    }
  }

  return withRoot(workspaceRoot, packages);
}
...
function npmPackagePath(
  ...
  if (
    ...
    return path.resolve(
      root,
      "node_modules",
      entry.resolved.slice("file:".length),
    );
  }
...
function runPmSync(pm: string, arguments_: string[], cwd: string) {
  if (process.platform === "win32") {
    return execFileSync([pm, ...arguments_].join(" "), {
      cwd,
      encoding: "utf8",
      shell: true,
    });
  }
  return execFileSync(pm, arguments_, { cwd, encoding: "utf8" });
}
function withRoot(root: string, packages: Package[]): Package[] {
  const members = packages.filter((entry) => entry.path !== root);
